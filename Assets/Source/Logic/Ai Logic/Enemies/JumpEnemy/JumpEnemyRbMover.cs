...
public class JumpEnemyRbMover : MonoCache{
    ...
    public void JumpToDirection(Vector3 direction){
        ...
        bodyBase.velocity += direction * jumpForce;
        StopGrab();
    }
    ...
    public void AccelerateTowardsPoint(Vector3 point, float power){
        if (_inStun) return;
        var damping = 0.05f;
        bodyBase.velocity += power * Time.deltaTime * (point-transform.position).normalized;
    ...
    public bool IsGrounded(){
        return Physics.Raycast(checkContactPoint.position, Vector3.down, 5 * transform.localScale.y, Layers.Environment);
    }
