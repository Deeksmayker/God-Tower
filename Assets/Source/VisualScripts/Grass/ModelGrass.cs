...
public class ModelGrass : MonoBehaviour {
    ...
    private int _currentChunkDensity;
    private float _currentLodCutoff;
    private float _currentDistanceCutoff;
    ...
    void OnEnable()
    {
        switch (SettingsController.GrassQualityValue)
        {
            case SettingsController.GrassQuality.Medium:
                _currentChunkDensity = chunkDensity / 2;
                _currentLodCutoff = lodCutoff / 2;
                _currentDistanceCutoff = distanceCutoff;
                break;
            case SettingsController.GrassQuality.Low:
                _currentChunkDensity = chunkDensity / 2;
                _currentLodCutoff = 0;
                _currentDistanceCutoff = 100;
                break;
            default:
                _currentChunkDensity = chunkDensity;
                _currentLodCutoff = lodCutoff;
                _currentDistanceCutoff = distanceCutoff;
                break;
        }

        numInstancesPerChunk = Mathf.CeilToInt(fieldSize / numChunks) * _currentChunkDensity;
        ...
        initializeGrassShader.SetInt("_Scale", _currentChunkDensity);
        ...
        /*initializeGrassShader.SetFloat("_WorldX", transform.position.x);
        initializeGrassShader.SetFloat("_WorldY", transform.position.y);
        initializeGrassShader.SetFloat("_WorldZ", transform.position.z);*/

        /*cullGrassShader.SetFloat("_WorldX", transform.position.x);
        cullGrassShader.SetFloat("_WorldY", transform.position.y);
        cullGrassShader.SetFloat("_WorldZ", transform.position.z);*/
        ...
        fieldBounds = new Bounds(transform.position, new Vector3(-fieldSize, displacementStrength * 2, fieldSize));
    }
    ...
    GrassChunk initializeGrassChunk(int xOffset, int yOffset) {
        ...
        chunk.bounds = new Bounds(c + transform.position, new Vector3(-chunkDim, 10.0f, chunkDim));
        ...
        initializeGrassShader.Dispatch(0, Mathf.CeilToInt(fieldSize / numChunks) * _currentChunkDensity, Mathf.CeilToInt(fieldSize / numChunks) * _currentChunkDensity, 1);
    ...
    void CullGrass(GrassChunk chunk, Matrix4x4 VP, bool noLOD) {
        ...
        cullGrassShader.SetFloat("_Distance", _currentDistanceCutoff);
    ...
    void Update() {
        ...
        for (int i = 0; i < numChunks * numChunks; ++i) {
            ...
            bool noLOD = dist < _currentLodCutoff;
    ...
    void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireCube(transform.position, new Vector3(fieldSize, 5, fieldSize));
